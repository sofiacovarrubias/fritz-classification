...
def get_short_IAUname(ztfname):
    ''' Info : Query the TNS name for any source
        Input : ZTFname
        Returns : TNS name wihtout AT/SN prefix
    '''
    fullname = get_IAUname(ztfname)
    if fullname.startswith("SN") or fullname.startswith("AT"):
        return fullname[3:]
    else:
        return fullname
...
def get_TNS_instrument_ID(inst):
    ...
    inst_ids = {'DBSP':1, 'ALFOSC': 41, 'LRIS': 3, 'DIS': 70, 'SEDM': 149, 'SPRAT': 156, 'GMOS': 6, 'Lick-3m': 10, 'LFC': 2, 'TSPEC': 109, 'NIRES': 252, 'GMOS_GS': 9, 'FLOYDS': 125, 'KAST': 10, 'Deveny+LMI':143, 'GHTS':127, 'NGPS':285}
